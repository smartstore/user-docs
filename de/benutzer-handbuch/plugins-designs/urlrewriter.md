...
Das Plugin verwendet eine an **Apache mod_rewrite** angelehnte Syntax mit reduziertem Funktionsumfang. So können Sie pro Zeile eine klare Regel hinterlegen und steuern das Verhalten der Weiterleitung über optionale Schalter.
...
* **Einfaches Redirect**: Alte URL &rarr; neue URL `/alte-seite /neue-seite [R=301,NC]`
...
Die weiteren Details zu den **Schaltern**, zum **Escaping** (welche Zeichen bei der Eingabe als Sonderzeichen zu behandeln sind) sowie zur Möglichkeit, Rewrite-Regeln **zusätzlich/optional in externen Dateien** bereitzustellen, finden Sie direkt im **Hilfe-Button (Konfiguration &rarr; Hilfe)**, da dort die relevanten Optionen, Dateinamen (für mod_rewrite bzw. IIS Rewrite) sowie das Regel-Vorrangprinzip zwischen Plugin-Regeln und Datei-Regeln verständlich zusammengefasst sind.
