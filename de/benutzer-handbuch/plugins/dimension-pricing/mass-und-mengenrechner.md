...
Sie richten den Rechner in einer [Maßvorlage](massvorlagen.md) ein. Eine Maßvorlage kann mehreren Produkten zugewiesen werden, sodass sie dieselbe Berechnungslogik verwenden. Konkrete Anwendungen aller Betriebsarten finden Sie unter [Praxisbeispiele](praxisbeispiele.md).

## Voraussetzungen

Bevor Sie den Rechner konfigurieren, sollte die Maßvorlage alle Maße enthalten, die in der Berechnung benötigt werden. Für jedes Maß legen Sie unter [**Maße**](massvorlagen.md#maße-hinzufügen) unter anderem Systemname, Minimalwert, Maximalwert, Standardwert und sichtbare Bezeichnung fest.
...
Die Bezeichnung der Artikelmenge übernimmt der Rechner aus der **Verpackungseinheit** des Produkts. Dabei verwendet er automatisch deren Einzahl oder Mehrzahl, zum Beispiel **1 Paket** oder **2 Pakete**.
...
| --- | --- | --- | --- | --- |
| Gesamtmaß eingeben – Artikelmenge berechnen | ein Gesamtwert | benötigte Artikelmenge | nicht änderbar | [Bodenfliesen](praxisbeispiele.md#bodenfliesen-pakete-aus-einer-gesamtfläche-berechnen) |
| Einzelmaße eingeben – Artikelmenge berechnen | einzelne Maße | Maßwert und benötigte Artikelmenge | nicht änderbar | [Vliestapete](praxisbeispiele.md#vliestapete-rollen-aus-wandmaßen-berechnen) |
| Einzelmaße eingeben – Preis berechnen | einzelne Maße | maßabhängiger Preis | keine Mengenauswahl | [Glasplatte](praxisbeispiele.md#glasplatte-preis-eines-zweidimensionalen-zuschnitts-berechnen) |
| Einzelmaße eingeben – Preis berechnen und Menge wählen | einzelne Maße und Artikelmenge | maßabhängiger Einzel- und Gesamtpreis | frei wählbar | [Akustikschaum](praxisbeispiele.md#akustikschaum-dreidimensionale-blöcke-konfigurieren) |
...
Eine vollständige Konfiguration zeigt das Beispiel [Bodenfliesen](praxisbeispiele.md#bodenfliesen-pakete-aus-einer-gesamtfläche-berechnen).
...
Der Preis basiert auf dem regulären Produktpreis und der berechneten Artikelmenge. Eine vollständige Konfiguration zeigt das Beispiel [Vliestapete](praxisbeispiele.md#vliestapete-rollen-aus-wandmaßen-berechnen).
...
Die konfigurierte Ausführung wird als ein Artikel in den Warenkorb gelegt; eine zusätzliche Mengenauswahl wird nicht angeboten. Beispiele für ein- und zweidimensionale Produkte finden Sie unter [Textilkabel](praxisbeispiele.md#textilkabel-preis-aus-einer-länge-berechnen) und [Glasplatte](praxisbeispiele.md#glasplatte-preis-eines-zweidimensionalen-zuschnitts-berechnen).
...
Beispiele für zwei- und dreidimensionale Produkte finden Sie unter [Tischplatte](praxisbeispiele.md#tischplatte-mehrere-identische-zuschnitte-bestellen) und [Akustikschaum](praxisbeispiele.md#akustikschaum-dreidimensionale-blöcke-konfigurieren).
...
Die Dezimalstellen und Schrittweiten der Eingabefelder legen Sie separat unter [**Maße**](massvorlagen.md#maße-hinzufügen) fest.
