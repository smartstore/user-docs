# Allgemeine Konzepte

Es gibt einige Konzepte, die wiederkehrend in verschiedenen Bereichen in **Smartstore** angewendet werden. Dieser Bereich beschäftigt sich mit diesen allgemeinen Konzepten.

  
{% columns %}
{% column width="50%" %}

{% content-ref url="./allgemeine-konzepte/zugriffsbeschrankungen-acl.md" %}
[Zugriffsbeschränkungen (ACL)](./allgemeine-konzepte/zugriffsbeschrankungen-acl.md)
{% endcontent-ref %}

{% content-ref url="./allgemeine-konzepte/html-inhalte-bearbeiten.md" %}
[HTML Inhalte bearbeiten](./allgemeine-konzepte/html-inhalte-bearbeiten.md)
{% endcontent-ref %}

{% content-ref url="./allgemeine-konzepte/mit-mehreren-sprachen-arbeiten.md" %}
[Mit mehreren Sprachen arbeiten](./allgemeine-konzepte/mit-mehreren-sprachen-arbeiten.md)
{% endcontent-ref %}

{% content-ref url="./allgemeine-konzepte/mit-mehreren-shops-arbeiten.md" %}
[Mit mehreren Shops arbeiten](./allgemeine-konzepte/mit-mehreren-shops-arbeiten.md)
{% endcontent-ref %}

{% content-ref url="./allgemeine-konzepte/seo.md" %}
[SEO](./allgemeine-konzepte/seo.md)
{% endcontent-ref %}

{% endcolumn %}
{% column %}

{% content-ref url="./allgemeine-konzepte/bilder-hochladen.md" %}
[Bilder hochladen](./allgemeine-konzepte/bilder-hochladen.md)
{% endcontent-ref %}

{% content-ref url="./allgemeine-konzepte/vorschau-fur-designs-shops.md" %}
[Vorschau für Designs & Shops](./allgemeine-konzepte/vorschau-fur-designs-shops.md)
{% endcontent-ref %}

{% content-ref url="./allgemeine-konzepte/als-pdf-herunterladen.md" %}
[Als PDF herunterladen](./allgemeine-konzepte/als-pdf-herunterladen.md)
{% endcontent-ref %}

{% content-ref url="./allgemeine-konzepte/tabellenansichten.md" %}
[Tabellenansichten](./allgemeine-konzepte/tabellenansichten.md)
{% endcontent-ref %}

{% endcolumn %}
{% endcolumns %}
