...
| **Pattern for file names** | Specifies the pattern for creating file names. The **Example** button allows you to preview the result of the pattern resolving.<br><br>**Supported placeholders:**<br>- **%Profile.Id%**: The numeric identifier of the export profile.<br>- **%Profile.FolderName%**: The name of the export folder for the profile.<br>- **%Profile.SeoName%**: The SEO friendly name of the export profile.<br>- **%ExportProfile.FolderName%**: The name of the export folder for the profile.<br>- **%Store.Id%**: The numeric identifier of the store of the current export run-through.<br>- **%Store.SeoName%**: The SEO friendly name of the store of the current export run-through. Resolves to `allstores` if the **Per store** option is activated.<br>- **%File.Index%**: A one based file index.<br>- **%Random.Number%**: A random number.<br>- **%Timestamp%**: A UTC timestamp.<br><br>**Note:** Placeholders are intended to easier create unique file names. Please pay attention to avoid patterrns that lead to non unique file names and thus file name conflicts. |
...
| **Email notification** | Sends a notification email about the completion of an export. Note that you can specify multiple email addresses where to send the email to.<br><br>**Note:** System exports (like product or category export) are always sending a notification email regardless of this option. |
